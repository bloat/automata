...
days for EJB and JSP, so my JavaScript knowledge is very minimal, so I
had new sympathy for people coming to Clojure with only minimal
experience of Java and the JVM.
...
Each one of these inputs is given a value which is a
power of two. These are also enumerated here and they are used to
convert from a numeric rule specification into a map of possible
